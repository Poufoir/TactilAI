...
─────
  ...
  python -m tactilai.scripts.train --resume --checkpoint-dir checkpoints/
...
from tactilai.training.selfplay import (
    CHECKPOINT_DIR,
    LOG_DIR,
    TOTAL_UPDATES,
    SelfPlayTrainer,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--updates", type=int, default=TOTAL_UPDATES,
    ...
    parser.add_argument(
        ...
        help="Device override: 'cpu', 'cuda'. Auto-detected if not set.",
    )
    parser.add_argument(
        "--resume", action="store_true",
        help="Resume training from existing checkpoints in checkpoint-dir",
    )
...
def main() -> None:
    ...
    try:
        ...
        print("\nInterrupted by user — saving final checkpoints...")
...
def _resume(trainer: SelfPlayTrainer, checkpoint_dir: str) -> None:
    """Loads latest checkpoints from pool if resuming."""
    if not trainer.pool_blue.is_empty:
        trainer.pool_blue.load_latest(trainer.agent_blue)
        print(f"Resumed blue agent from {trainer.pool_blue._pool[-1].name}")
    if not trainer.pool_red.is_empty:
        trainer.pool_red.load_latest(trainer.agent_red)
        print(f"Resumed red agent from {trainer.pool_red._pool[-1].name}")
